...
module.exports = function (opts_) {
  ...
  , port: opts_.port
  , name: opts_.name
  , verbose: opts_.verbose || false
  , dispatcher: opts_.dispatcher
  };
  
  logger = coolog.logger(__filename.from(__filename.lastIndexOf('/') + 1), 'cipolla');
  ...
  if (cluster.isMaster) {
    ...
    (function _workr() {
      ...
      if (config.verbose === true) {
        app.use(function _annoy(req, res, next) {
          logger.debug('' + req.method.toUpperCase().bold + ' ' + req.url);
          next();
        });
      }
      ...
      app.use(function _404handler(req, res) {
        dispatcher.emit('404', req, res);
      });
...
module.exports.forever = function (mainScript, watchDirectory) {
  ...
  var forever_logger = coolog.logger('forever', 'cipolla');
